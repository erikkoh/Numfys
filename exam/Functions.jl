...
function tempreture_trend_for_energy_and_mag(;L::Int=40, num_sweeps::Int=5_000, temps = range(1.0, 3.0, length= 100),possible_indexes = [])
    ...
    for (i, ts) in ProgressBar(enumerate(temps))
        ...
        heat_capacity[i] = variance_dE[i] / (ts^2)/(L^2)  # Calculate heat capacity
    end
    argmax_heat_capacity = argmax(heat_capacity[10:end])  # Find the index of the maximum heat capacity
...
function ploting_heat_mag_and_energy()
    onsager_mag = t -> (1 - sinh(2/t)^(-4))^(1/8)*40^2
    ...
    scatter(temps, magnetization, xlabel="Temperature", ylabel="Magnetization", label="Mangetisation Numerical")
    temps_onsager = findall(t-> t < 2.27, temps)
    plot!(temps[temps_onsager], onsager_mag.(temps[temps_onsager]), label="Magnetisation Onsager Solution", color=:red)
    scatter!(legend=:bottomleft)
...
function plot_L_dependence_of_alpha(num_sweeps::Int=10_000)
    ...
    max_heat = [maximum(system_heat_capcity[10:end]) for system_heat_capcity in heat_capacity_L]
...
function data_collaps_of_susptibility(onsager = false, Hs = [0.01, 0.02 , 0.03, 0.05])
    critical_temp = 2.3
    gamma = 7/4
    beta = 1/8
    delta = 15
    ...
    scatter!(p1, log.(h_scaled[after_critical]), log.(susceptibility_scaled[after_critical]),
            label="H=$H T>T_c",
            xlabel="log(H scaled)", ylabel="log(Susceptibility scaled)", color=:blue, )
    for H in Hs[2:end]
        temps, _, susceptibility = calc_suseptibility(H, temps)
